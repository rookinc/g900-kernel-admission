...
\item The recorded sibling signing is not switching-equivalent to the canonical signing.
...
\begin{proof}
The first two items are direct finite consequences of the construction kernel. The remaining items are certified by explicit certificate files generated from the finite payload. The QED status ledger records six direct kernel claims and seven certified claims, with no planned or uncertified claims remaining in the bounded claim set. The certificate dependencies are summarized in the following sections.
\end{proof}
